...
export interface TruncateSlowIndexesRequest {
    organisationId: string;
    kbId: string;
    sessionId: string;
    aPIVersion?: TruncateSlowIndexesAPIVersionEnum;
}
...
export class LanguageApi extends BaseAPI {
     ...
     */
    truncateSlowIndexes({ organisationId, kbId, sessionId, aPIVersion }: TruncateSlowIndexesRequest): Observable<JsonMessage>
    truncateSlowIndexes({ organisationId, kbId, sessionId, aPIVersion }: TruncateSlowIndexesRequest, opts?: OperationOpts): Observable<AjaxResponse<JsonMessage>>
    truncateSlowIndexes({ organisationId, kbId, sessionId, aPIVersion }: TruncateSlowIndexesRequest, opts?: OperationOpts): Observable<JsonMessage | AjaxResponse<JsonMessage>> {
        throwIfNullOrUndefined(organisationId, 'organisationId', 'truncateSlowIndexes');
        throwIfNullOrUndefined(kbId, 'kbId', 'truncateSlowIndexes');
        throwIfNullOrUndefined(sessionId, 'sessionId', 'truncateSlowIndexes');
        ...
        return this.request<JsonMessage>({
            url: '/api/language/truncate-slow-indexes/{organisationId}/{kbId}'.replace('{organisationId}', encodeURI(organisationId)).replace('{kbId}', encodeURI(kbId)),
            method: 'GET',
            headers,
        }, opts?.responseOpts);
    };
 ...
 * @enum {string}
 */
export enum TruncateSlowIndexesAPIVersionEnum {
    _1 = '1'
}
/**
 * @export
