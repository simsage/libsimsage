...
/// struct for typed errors of method [`crawler_external_document_un_changed`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CrawlerExternalDocumentUnChangedError {
    Status500(crate::models::JsonMessage),
    UnknownValue(serde_json::Value),
}
...
pub async fn crawler_external_document_un_changed(configuration: &configuration::Configuration, cm_document_un_changed: crate::models::CmDocumentUnChanged, api_version: Option<&str>) -> Result<crate::models::JsonMessage, Error<CrawlerExternalDocumentUnChangedError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/crawler/external/document/un-changed", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&cm_document_un_changed);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CrawlerExternalDocumentUnChangedError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
