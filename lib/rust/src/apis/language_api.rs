...
/// struct for typed errors of method [`truncate_slow_indexes`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum TruncateSlowIndexesError {
    Status500(crate::models::JsonMessage),
    UnknownValue(serde_json::Value),
}
...
pub async fn truncate_slow_indexes(configuration: &configuration::Configuration, organisation_id: &str, kb_id: &str, session_id: &str, api_version: Option<&str>) -> Result<crate::models::JsonMessage, Error<TruncateSlowIndexesError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/language/truncate-slow-indexes/{organisationId}/{kbId}", local_var_configuration.base_path, organisationId=crate::apis::urlencode(organisation_id), kbId=crate::apis::urlencode(kb_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<TruncateSlowIndexesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
