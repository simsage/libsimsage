...
pub async fn system_logs(configuration: &configuration::Configuration, organisation_id: &str, year: i32, month: i32, day: i32, hour: i32, hours: i32, session_id: &str) -> Result<Vec<crate::models::CmLoggerEntry>, Error<SystemLogsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/stats/system-logs/{organisationId}/{year}/{month}/{day}/{hour}/{hours}", local_var_configuration.base_path, organisationId=crate::apis::urlencode(organisation_id), year=year, month=month, day=day, hour=hour, hours=hours);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
