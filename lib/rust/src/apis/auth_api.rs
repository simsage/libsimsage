...
/// struct for typed errors of method [`group_edit_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GroupEditInfoError {
    Status500(crate::models::JsonMessage),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`sign_in_admin_using_session_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SignInAdminUsingSessionIdError {
    Status500(crate::models::JsonMessage),
    UnknownValue(serde_json::Value),
}
...
pub async fn group_edit_info(configuration: &configuration::Configuration, organisation_id: &str, session_id: &str, cm_group_edit_info: crate::models::CmGroupEditInfo) -> Result<crate::models::CmGroupList, Error<GroupEditInfoError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/auth/group-edit-info/{organisationId}", local_var_configuration.base_path, organisationId=crate::apis::urlencode(organisation_id));
    ...
    local_var_req_builder = local_var_req_builder.header("session-id", session_id.to_string());
    local_var_req_builder = local_var_req_builder.json(&cm_group_edit_info);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GroupEditInfoError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn sign_in_admin_using_session_id(configuration: &configuration::Configuration, session_id: &str) -> Result<crate::models::SignInAdmin, Error<SignInAdminUsingSessionIdError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/auth/admin/authenticate/session-id", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SignInAdminUsingSessionIdError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
