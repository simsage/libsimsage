/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package org.openapitools.client.models

import org.openapitools.client.models.CMSynonym

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * A paginated synonym return list matching a request for the same.
 *
 * @param organisationId the organisation (its guid id).
 * @param kbId the knowledge-base (its guid id).
 * @param filter The filter that was used to find these items.
 * @param synonymList the resulting list of synonyms (return)
 * @param numSynonyms the total number of synonyms that matched.
 */


data class CMSynonymsPaginatedResult (

    /* the organisation (its guid id). */
    @Json(name = "organisationId")
    val organisationId: kotlin.String,

    /* the knowledge-base (its guid id). */
    @Json(name = "kbId")
    val kbId: kotlin.String,

    /* The filter that was used to find these items. */
    @Json(name = "filter")
    val filter: kotlin.String,

    /* the resulting list of synonyms (return) */
    @Json(name = "synonymList")
    val synonymList: kotlin.collections.List<CMSynonym>,

    /* the total number of synonyms that matched. */
    @Json(name = "numSynonyms")
    val numSynonyms: kotlin.Int

)

